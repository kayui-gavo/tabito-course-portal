...
async function checkPage(page,label,url,viewport){
  ...
  try{
    ...
    const state=await page.evaluate(()=>{
      ...
      const canvas=(window.INFORMATION_CANVAS_TEXT_AUDIT_V13||{})[id]||null;
      const layout=window.INFORMATION_LAYOUT_AUDIT_V13||null;
      const pageAudit=window.INFORMATION_PAGE_AUDIT_V13||null;
      return {
        id,
        ...
        layout,
        pageAudit,
        figures:document.querySelectorAll('.scientific-figure-v12').length,
        oldFigures:document.querySelectorAll('.scientific-figure-v11').length,
        duplicateNav:document.querySelectorAll('.lesson-nav-v13').length,
        visibleOldProgress:[...document.querySelectorAll('.lesson-reading-progress')].filter(n=>getComputedStyle(n).display!=='none').length
      };
    });
    const problems=[];
    if(state.overflow>4)problems.push(`global horizontal overflow ${state.overflow}px`);
    if(jsErrors.length)problems.push(`pageerror: ${jsErrors.join(' | ')}`);
    if(state.oldFigures)problems.push(`legacy v11 figure still rendered: ${state.oldFigures}`);
    if(url.includes('lesson.html')){
      if(state.figures!==1)problems.push(`scientific figure count ${state.figures}, expected 1`);
      ...
      if(state.layout?.duplicateIds?.length)problems.push(`duplicate ids: ${state.layout.duplicateIds.join(',')}`);
    }else{
    ...
    failures.push({label,url,viewport,problems:[String(error?.stack||error)]});
...
for(const id of mainIds)await checkPage(page,`desktop-${id}`,`${base}lesson.html?id=${id}`,{width:1440,height:1000});
for(const id of programIds)await checkPage(page,`desktop-${id}`,`${base}lesson.html?id=${id}`,{width:1440,height:1000});
