...
(() => {
  ...
  if(page==='programming.html'){
    ...
    if(lead)lead.textContent='初級14講・中級19講・上級15講を教材順で進めます。各講でコード行の役割を読み、実際のPythonをブラウザで実行します。さらに10進→2進、中央値、素数・素因数分解、FizzBuzz、グラフ理論、ソート、待ち行列、JAN、すごろくなど、状態変化を追う価値が高い11講は専用図版で処理を可視化し、重要講では図の直後に追跡インタラクティブを置いています。';
    insertStrip(document.querySelector('.programming-guide'),[
      ['48','全講を教材順で','初級→中級→上級の難度設計を維持'],
      ['11','状態追跡の専用図版','配列・分岐・反復・行列を途中状態まで可視化'],
