...
(() => {
  ...
  if(page==='programming.html'){
    ...
    if(lead)lead.textContent='初級14講・中級19講・上級15講を教材順で進めます。各講でコード行の役割を読み、実際のPythonをブラウザで実行します。特に第29〜48講のうち、ソート、append、2次元配列、辞書、金額分解、区分料金、割引、集計、じゃんけん、確率・統計、素数、FizzBuzz、グラフ理論、待ち行列、誤り検出、すごろくなど20講は専用図版で途中状態まで可視化し、重要講には図直結の追跡インタラクティブも置いています。';
    insertStrip(document.querySelector('.programming-guide'),[
      ['48','全講を教材順で','初級→中級→上級の難度設計を維持'],
      ['20','状態追跡の専用図版','配列・分岐・反復・行列・料金計算を途中状態まで可視化'],
