...
(() => {
  ...
  const configs={
    ...
    p44:{
      ...
      draw(ctx,k){const {text,wrap,rr,box,arrow,head}=k;head(ctx,'バブルソート・選択ソート','同じ最終結果だけでなく、1周の「比較のしかた」と「確定する側」を見比べる。');const start=[64,34,25,12,22,11,90],bubble=[34,25,12,22,11,64,90],select=[11,34,25,12,22,64,90],final=[11,12,22,25,34,64,90];const row=(y,arr,label,fixed=-1,color=C.blue)=>{text(ctx,label,45,y+28,11,C.gray,700);arr.forEach((v,i)=>{const x=190+i*125,done=i===fixed;rr(ctx,x,y,100,58,done?'#eef6fa':'#fff',done?'#bfd1db':'#d7e1e6',7);text(ctx,String(v),x+50,y+29,15,done?color:C.navy,700,'center','middle');});};
