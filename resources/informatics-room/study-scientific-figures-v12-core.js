/* 情報Ⅰ v13 — 教材級 Canvas 図版共通基盤
   v12 API互換のまま、文字のはみ出し・箱内重なり・極端な縮小を防ぐ。 */
(() => {
  ...
  function availableWidth(ctx,x,align='left',pad=8){
    const W=logicalWidth(ctx);
    if(align==='center')return Math.max(1,2*Math.min(Math.max(0,x-pad),Math.max(0,W-x-pad)));
    if(align==='right'||align==='end')return Math.max(1,x-pad);
    return Math.max(1,W-x-pad);
  }
  function fitSize(ctx,s,start,min,maxW,weight=400){
    let size=start;font(ctx,size,weight);
    while(size>min&&ctx.measureText(String(s)).width>maxW){size=Math.max(min,size-.5);font(ctx,size,weight);}
    return size;
  }
  function splitLines(ctx,s,maxW,size,weight=400){
    font(ctx,size,weight);const out=[];
    String(s).split('\n').forEach((para,pi)=>{
      if(para===''){out.push('');return;}
      let line='';
      for(const ch of para){const test=line+ch;if(line&&ctx.measureText(test).width>maxW){out.push(line);line=ch;}else line=test;}
      if(line)out.push(line);
      if(pi<String(s).split('\n').length-1&&para!==''&&out.at(-1)!=='')out.push('');
    });
    return out;
  }
  function ellipsize(ctx,s,maxW,size,weight=400){
    font(ctx,size,weight);let out=String(s);if(ctx.measureText(out).width<=maxW)return out;
  ...
  function text(ctx,s,x,y,size=14,color=C.dark,weight=400,align='left',base='alphabetic',maxWidth=null,minSize=8.5){
    ...
    const cap=maxWidth||availableWidth(ctx,x,align,8);
    let drawSize=size;
    ...
    const lh=Math.max(drawSize*1.35,drawSize+3);
    let startY=y;
    if(lines.length>1&&base==='middle')startY=y-(lines.length-1)*lh/2;
    ctx.fillStyle=color;font(ctx,drawSize,weight);ctx.textAlign=align;ctx.textBaseline=base;
    lines.forEach((line,i)=>{if(line)ctx.fillText(line,x,startY+i*lh);});
    return drawSize;
  }
  ...
  function wrap(ctx,s,x,y,w,lh=19,size=12,color=C.gray,weight=400,maxH=Infinity,minSize=8.5){
    ...
    const maxLines=Number.isFinite(maxH)?Math.max(1,Math.floor(maxH/lineHeight)):lines.length;
    let clipped=false;
    ...
    if(drawSize<size-2.5)qa(ctx,'shrunk',{text:String(s).slice(0,80),from:size,to:drawSize});
    if(clipped)qa(ctx,'truncated',{text:String(s).slice(0,120),width:w,height:maxH});
    ctx.fillStyle=color;font(ctx,drawSize,weight);ctx.textAlign='left';ctx.textBaseline='top';
    let yy=y;lines.forEach(line=>{if(line)ctx.fillText(line,x,yy);yy+=lineHeight;});return yy;
  }

  function rr(ctx,x,y,w,h,fill=C.white,stroke=C.grid,r=10,lw=1.2){
    ctx.beginPath();
  ...
  function box(ctx,x,y,w,h,h1,p='',opt={}){
    rr(ctx,x,y,w,h,opt.fill||C.white,opt.stroke||C.grid,opt.r||10,opt.lw||1.2);
    const pad=13,titleSize=fitSize(ctx,h1,opt.ts||14,opt.minTitleSize||9.5,w-pad*2,700);
    if(titleSize<(opt.ts||14)-2.5)qa(ctx,'shrunk',{text:String(h1).slice(0,80),from:opt.ts||14,to:titleSize,scope:'box-title'});
    text(ctx,h1,x+pad,y+22,titleSize,opt.tc||C.navy,700,'left','alphabetic',w-pad*2,opt.minTitleSize||9.5);
    if(p){
      const bodyTop=y+35,bodyBottom=y+h-9,bodyH=Math.max(18,bodyBottom-bodyTop);
      wrap(ctx,p,x+pad,bodyTop,w-pad*2,opt.lh||18,opt.bs||11,opt.bc||C.gray,400,bodyH,opt.minBodySize||8.8);
    }
  }
  ...
  function head(ctx,h,p){
    const titleSize=fitSize(ctx,h,24,17,1130,700);text(ctx,h,34,42,titleSize,C.navy,700,'left','alphabetic',1130,17);
    wrap(ctx,p,34,58,1130,18,12,C.gray,400,42,10);
  }
  ...
  function cell(ctx,x,y,w,h,s,opt={}){
    ...
    const weight=opt.head?700:400,color=opt.head?C.navy:C.dark,start=opt.fs||10,min=opt.minFs||7.5,pad=8;
    let size=fitSize(ctx,s,start,min,w-pad*2,weight);
    if(ctx.measureText(String(s)).width<=w-pad*2){text(ctx,s,x+w/2,y+h/2,size,color,weight,'center','middle',w-pad*2,min);return;}
    const lines=splitLines(ctx,s,w-pad*2,min,weight).filter(v=>v!=='').slice(0,2),lh=Math.max(min*1.35,min+2),cy=y+h/2-(lines.length-1)*lh/2;
    if(lines.length>=2)qa(ctx,'wrappedCell',{text:String(s).slice(0,80),width:w,height:h});
    lines.forEach((lineText,i)=>text(ctx,lineText,x+w/2,cy+i*lh,min,color,weight,'center','middle',w-pad*2,min));
  }
  ...
  function setup(canvas,w=1200,h=660){const dpr=DPR();canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);canvas.style.aspectRatio=`${w}/${h}`;const ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);ctx.lineCap='round';ctx.lineJoin='round';ctx.__logicalW=w;ctx.__logicalH=h;ctx.__textQa={shrunk:[],truncated:[],wrappedCell:[]};return ctx;}
  ...
  function renderCanvas(canvas,config){const ctx=setup(canvas,config.width||1200,config.height||660);config.draw(ctx,{C,text,wrap,rr,box,line,arrow,head,cell,table,axis,w:config.width||1200,h:config.height||660});canvas.__figureTextQa=ctx.__textQa;return ctx.__textQa;}
  function saveQa(fig,report){
    const id=fig.dataset.figureV12||fig.dataset.programFigureV12||fig.dataset.programFigureV12b||fig.dataset.programFigureV12c||fig.dataset.programFigureV12d||fig.dataset.programFigureV12e||fig.dataset.programFigureV12f||fig.dataset.programFigureV12g||'unknown';
    const all=window.INFORMATION_CANVAS_TEXT_AUDIT_V13=window.INFORMATION_CANVAS_TEXT_AUDIT_V13||{};all[id]=report;
    fig.dataset.textLayoutIssue=(report.truncated.length||report.shrunk.some(x=>x.to<9))?'1':'0';
  }
