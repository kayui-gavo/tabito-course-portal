...
(() => {
  ...
  function init(){
    ...
    items.forEach((item,i)=>{if(!item.node.id)item.node.id=`study-section-v12-${i}`;});
  ...
  window.renderStudyLesson=function renderLessonNavV12(){baseRender();init();};
})();