...
(() => {
  ...
  function enhance(){
    document.querySelectorAll('.et-source-practice-v7-list article').forEach((article,index)=>{
      ...
      details.addEventListener('toggle',()=>{if(details.open&&ta.value.trim().length<12){block.classList.add('needs-writing');ta.setAttribute('aria-describedby',`practice-hint-${index}`);}else block.classList.remove('needs-writing');});
    });
  }
