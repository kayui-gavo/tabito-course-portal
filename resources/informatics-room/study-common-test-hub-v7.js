...
(() => {
  function init(){
    ...
    intro.insertAdjacentHTML('afterend',`<section class="ct-hub-v7">
      <div class="ct-hub-v7-head"><span>COMMON TEST 2026</span><h2>令和8年度は4大問。知識を場面の中で使う</h2><p>大学入試センターの高校教員評価では、教科書の基本知識に加え、日常・社会の場面や初見資料を読み、複数の知識を組み合わせて問題を発見・解決し、方法そのものを評価する力が重視されています。</p></div>
      <div class="ct-hub-v7-grid">
        ...
        <div><b>第3問｜プログラム</b><p>待ち時間などのモデルを数値で確認し、配列・条件・反復を追跡し、探索の打切りなど処理の改善まで考える。</p></div>
        <div><b>第4問｜データ分析</b><p>オープンデータ、欠損、グラフ、散布図、相関、箱ひげ図、回帰を組み合わせ、分析・補正方法が目的に合うか評価する。</p></div>
      </div>
      <p class="ct-hub-v7-source">大学入試センター「令和8年度 大学入学共通テスト問題評価・分析委員会報告書（『情報Ⅰ』高等学校評価）」の評価内容を学習用に要約。</p>
    </section>`);
  }
