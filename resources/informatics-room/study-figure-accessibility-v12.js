...
(() => {
  ...
  function enhanceFigure(fig,index){
    ...
    if(canvas){canvas.setAttribute('role','img');canvas.setAttribute('aria-labelledby',titleId);canvas.setAttribute('aria-describedby',descId);}
  ...
  function enhanceLiveRegions(){
    document.querySelectorAll('.pf-result-v12c,.pf-score-stats-v12c,.figure-lab-result-v12,[data-program-run-output]').forEach(node=>{if(!node.hasAttribute('aria-live')){node.setAttribute('aria-live','polite');node.setAttribute('aria-atomic','true');}});
  }
  function init(){document.querySelectorAll('.scientific-figure-v12').forEach(enhanceFigure);enhanceLiveRegions();}
