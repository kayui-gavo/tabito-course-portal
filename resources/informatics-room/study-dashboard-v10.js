...
(() => {
  ...
  function currentStats(){
    ...
    return {completed:lessons.filter(l=>completed.has(l.id)).length,total:lessons.length,masteryDone,masteryTotal,wrong:Array.isArray(wrong)?wrong.length:0,glossaryReview,figureReviewCount,next};
  }
  ...
  function init(){
    ...
    const recommendation=s.wrong>0?'新しいPARTへ進む前に、要復習問題を5〜10問だけ解き直す。':s.figureReviewCount>0?`「要復習」にした図版が${s.figureReviewCount}件あります。図を見ずに関係・数値・方向を説明してから、答えと比較する。`:s.glossaryReview>0?'用語を5語ほど思い出してから次のPARTへ進む。':'次のPARTを読み、図を自分の言葉で説明してから到達チェック・問題演習へ進む。';
    curriculum.insertAdjacentHTML('beforebegin',`<section class="study-dashboard-v10">
      ...
      <footer><b>おすすめ</b><p>${recommendation}${s.figureReviewCount?`　図版要復習：${s.figureReviewCount}件。`:''}</p></footer>
    </section>`);
  }
