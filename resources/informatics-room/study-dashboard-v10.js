...
(() => {
  ...
  const FIGURE_REVIEW='tabito-info-figure-review-v12';
  const PRACTICE_REVIEW='tabito-info-practice-status-v13';
  ...
  function currentStats(){
    ...
    const figureReview=safe(FIGURE_REVIEW,{});
    const practiceReview=safe(PRACTICE_REVIEW,{});
    ...
    const figureReviewCount=Object.values(figureReview).filter(v=>v==='review').length;
    const practiceReviewCount=Object.values(practiceReview).filter(v=>v==='review').length;
    const next=lessons.find(l=>!completed.has(l.id))||lessons[0];
    return {completed:lessons.filter(l=>completed.has(l.id)).length,total:lessons.length,masteryDone,masteryTotal,wrong:Array.isArray(wrong)?wrong.length:0,glossaryReview,figureReviewCount,practiceReviewCount,next};
  }
  ...
  function init(){
    ...
    const recommendation=s.wrong>0?'新しいPARTへ進む前に、要復習問題を5〜10問だけ解き直す。':s.practiceReviewCount>0?`「もう一度解く」にした記述演習が${s.practiceReviewCount}問あります。解答例を閉じた状態で、自分の解答をもう一度書く。`:s.figureReviewCount>0?`「要復習」にした図版が${s.figureReviewCount}件あります。図を見ずに関係・数値・方向を説明してから、答えと比較する。`:s.glossaryReview>0?'用語を5語ほど思い出してから次のPARTへ進む。':'次のPARTを読み、図を自分の言葉で説明してから到達チェック・問題演習へ進む。';
    curriculum.insertAdjacentHTML('beforebegin',`<section class="study-dashboard-v10">
      ...
      <footer><b>おすすめ</b><p>${recommendation}${s.practiceReviewCount?`　記述演習：${s.practiceReviewCount}問。`:''}${s.figureReviewCount?`　図版：${s.figureReviewCount}件。`:''}</p></footer>
    </section>`);
  }
