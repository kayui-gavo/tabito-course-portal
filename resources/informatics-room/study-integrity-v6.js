...
(() => {
  ...
  const middleIds=programming.filter(x=>x.level==='中級').map(x=>x.id);
  const advancedIds=programming.filter(x=>x.level==='上級').map(x=>x.id);
  const missingMiddle=middleIds.filter(id=>!middle[id]);
  ...
  const figureAudit=window.INFORMATION_FIGURE_AUDIT_V12||{};
  const programFigureAudit=window.INFORMATION_PROGRAM_FIGURE_AUDIT_V12||{};
  const report={
    ...
    programmingFigureCovered:programFigureAudit.coveredCount||0,programmingFigureExpected:programFigureAudit.expectedCount||0,programmingFigureMissing:programFigureAudit.missing||[],programmingFigureDuplicate:programFigureAudit.duplicate||[],programmingFigureInteractiveThird:window.PROGRAM_FIGLAB_V12C_COUNT||0,
  ...
  const figureProblem=report.mainFigureCovered!==47||report.mainFigureMissing.length||report.mainFigureDuplicate.length||report.programmingFigureCovered!==20||report.programmingFigureExpected!==20||report.programmingFigureMissing.length||report.programmingFigureDuplicate.length||(window.PROGRAM_FIGLAB_V12C_COUNT||0)!==7;
