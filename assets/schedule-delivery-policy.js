(() => {
  ...
  function patchOverview(){document.querySelectorAll('#courseOverviewBody tr').forEach(row=>{const name=row.querySelector('.course-overview-name strong')?.textContent?.trim();if(name==='国语'){const meta=row.querySelector('.course-overview-name span');if(meta&&meta.textContent!=='10/9起周五 13:40–16:40')meta.textContent='10/9起周五 13:40–16:40';}});}
  ...
  const calendar=document.querySelector('.office-calendar');if(calendar&&'MutationObserver'in window){let queued=false;new MutationObserver(mutations=>{if(!mutations.some(m=>m.type==='childList')||queued)return;queued=true;requestAnimationFrame(()=>{queued=false;applyAll(calendar);});}).observe(calendar,{childList:true,subtree:true});}
  const overview=document.getElementById('courseOverviewBody');if(overview&&'MutationObserver'in window){let queued=false;new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;patchOverview();});}).observe(overview,{childList:true,subtree:true});}
})();